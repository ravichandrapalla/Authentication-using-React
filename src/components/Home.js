...
const Home = () => {
  ...
  const checkEmail = (e) => {
    ...
    if (email.length <= 12) {
      ...
      !email.split("@")[1] === ("gmail.com" || "outlook.com" || "yahoo.com")
    ) {
      setValidEmail(false);
    } else {
      setValidEmail(true);
  ...
  const onSubmit = () => {
    if (
      ...
      setGreenAlert(false);
      if (!validName) {
        alert("Name should consists of atleast 3 chars and a space");
      } else if (!validEmail) {
        alert("email should have '@' and '.com' as per mail standards");
      } else if (password !== confirmPassword) {
        alert("passward and confirm passward mismatch");
      } else if (password.length < 5) {
        alert("passward length should be atleast 5");
      }
    } else {
      setRedAlert(false);
      setGreenAlert(true);
